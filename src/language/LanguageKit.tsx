...
class MultiLangManager {
    ...
    init = async () => {
        return AsyncStorage.getItem(LANGUAGE_STORE_KEY).then(value => {
            console.log({ value });
            if (value) {
...
export const withMultiLang = (WrappedComponent: any) => {
    return class extends Component<any, MultiLangState> {
        ...
        setDisplayLang = (langKey: String) => {
            if (this.state.langKey === langKey) return;
            this.setState({
                langKey
            });
        };
        ...
        render = () => {
            return (
                <WrappedComponent
                    ...
                    setCurrLang={this.setCurrLang}
                    strings={multiLangManager.localizedStrings}
                    {...this.props}
                />
            );
        };
